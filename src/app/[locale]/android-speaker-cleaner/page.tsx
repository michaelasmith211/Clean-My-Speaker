...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const alternates = getHreflangAlternates('/android-speaker-cleaner');
  const canonical = getCanonicalUrl('/android-speaker-cleaner', locale);

  return {
    title: t('androidCleaner.metaTitle'),
    description: t('androidCleaner.metaDesc'),
    alternates: { canonical, languages: alternates },
    openGraph: { title: t('androidCleaner.metaTitle'), description: t('androidCleaner.metaDesc'), url: canonical },
  };
}

export default function AndroidCleanerPage({ params }: PageProps) {
  const locale = isLocaleSupported(params.locale) ? params.locale : DEFAULT_LOCALE;
  const { t } = getTranslations(locale);
  const breadcrumbs = [{ name: t('nav.android'), href: '/android-speaker-cleaner' }];
    ...
    <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 text-start">
      ...
      <header className="space-y-4 my-8">
        <div className="inline-block px-3 py-1 rounded-full bg-slate-800 border border-slate-700 text-sky-400 text-xs font-semibold uppercase tracking-wider">
          Android Acoustic Moisture Clearance
        </div>
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight">
          {t('androidCleaner.h1')}
        </h1>
        <p className="text-base sm:text-lg text-slate-300 leading-relaxed">
          {t('androidCleaner.sub')}
        </p>
      </header>
      ...
      <RelatedGuides currentPath="/android-speaker-cleaner" locale={locale} />
    </main>
  );
}
